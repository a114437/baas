...
public class WebSocket extends WebSocketClient implements Observerable{
...
//        new TransactionsImpl(JsonUtils.toJsonString(map)).run();
		Map result = (Map) map.get("transaction");
		if(result.get("Amount")==null) {
			return;
		}
		String Fee = (String) result.get("Fee");
		Fee = new BigDecimal(Fee).divide(new BigDecimal("1000000")).toString();
		result.put("Fee", Fee);
		try {
			Map amount = (Map) result.get("Amount");
		} catch (Exception e) {
			String amount = (String) result.get("Amount");
			Map<String, Object> amountData = new HashMap<String, Object>();
			amountData.put("currency", "SWT");
			amountData.put("issuer", "");
			amountData.put("value", new BigDecimal(amount).divide(new BigDecimal("1000000")).toString());
			result.put("Amount", amountData);
		}
    	notifyObserver(JsonUtils.toJsonString(result));
    }
	...
	public void notifyObserver(String message) {
		...
		for (int i = 0; i < list.size(); i++) {
			list.get(i).updateTransactions(message);
		}
	}
}
